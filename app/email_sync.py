    ...
    def fetch_emails(self, last_email_date=None):
        ...
        for msg in messages:
            txt = self.service.users().messages().get(
                userId='me', 
                id=msg['id'],
                format='full'  # ensures body is included
            ).execute()
            ...
            # Parse date
            internal_date_ms = txt.get('internalDate')  # comes as a string e.g. "1554492714000"
            received_date = None
            if internal_date_ms:
                received_date = datetime.fromtimestamp(int(internal_date_ms) / 1000, tz=timezone.utc)
